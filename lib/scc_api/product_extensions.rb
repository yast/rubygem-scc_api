module SccApi
  class ProductExtensions
    ...
    def self.from_hash(extensions_response)
      extensions = extensions_response.map do |extension|
        ...
        Extension.new(extension["zypper_name"], extension["name"],
          extension["arch"], extension["zypper_version"], free: extension["free"])
      end
  ...
  class Extension
    ...
    attr_reader :short_name, :long_name, :description, :product_ident, :free, :arch, :version

    def initialize(product_ident, short_name, arch, version, long_name: "", description: "", free: false)
      @arch = arch
